      ...
      {screen !== 'onboarding' && screen !== 'test' && (
        ...
        >
          ...
          {role === 'student' && screen === 'learning' && learningSection === 'flashcards' && (
            <Flashcards onBack={() => setLearningSection(null)} />
          )}
          {role === 'student' && screen === 'learning' && learningSection === 'library' && (
            <LibraryView onBack={() => setLearningSection(null)} />
          )}
          ...
          {role === 'student' && screen === 'learning' && learningSection === 'professor' && (
            <AIProfessor onBack={() => setLearningSection(null)} user={user} />
          )}
...
function AIProfessor({ onBack, user }) {
  const topics = [
  ...
  const handleUserMessage = async (text) => {
    ...
    const systemPrompt = `Du bist Professorin Dr. Weber, eine warmherzige, erfahrene Pflegeprofessorin in Deutschland. Du sprichst mit ${user.firstName || 'der Schülerin'}, einer angehenden Pflegekraft aus ${user.country || 'dem Ausland'} auf Niveau ${user.level}.

Thema der heutigen Unterhaltung: ${topic.label} (${topic.desc}).

Wichtig:
- Antworte IMMER auf Deutsch.
- Halte Antworten SEHR KURZ (2–3 Sätze, max. 50 Wörter), weil sie vorgelesen werden.
- Sprich natürlich und warm, wie im echten Gespräch.
- Nutze einfache, klare Sprache passend zu Niveau ${user.level}.
- Stelle am Ende oft eine Rückfrage, um das Gespräch am Laufen zu halten.
- Korrigiere Grammatik dezent im Kontext (nicht direkt "Fehler!" sagen).
- Vermeide Listen, Aufzählungen, Markdown oder Sonderzeichen — nur fließender Text.`;
  ...
  const reset = () => {
    setTopic(null); setTranscript([]); setCurrentUser(''); setError(null);
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 780, margin: '20px auto 0' }}>
        ...
        <div className="paper-card" style={{ padding: 24, minHeight: 360, maxHeight: 420, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
